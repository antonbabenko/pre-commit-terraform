...
```bash
brew install pre-commit terraform-docs tflint tfsec trivy checkov terrascan infracost tfupdate minamijoyo/hcledit/hcledit jq flock
```
...
| `terraform_providers_lock`                             | Updates provider signatures in [dependency lock files](https://www.terraform.io/docs/cli/commands/providers/lock.html). [Hook notes](#terraform_providers_lock)                                                                              | `flock` recommended (fallbacks to pure shell implementation if `flock` isn't found)                                                 |
...
| `terraform_validate`                                   | Validates all Terraform configuration files. [Hook notes](#terraform_validate)                                                                                                                                                               | `jq` only for `--retry-once-with-cleanup` flag; `flock` recommended (fallbacks to pure shell implementation if `flock` isn't found) |
