...
## Available Hooks
Please note, that only first *two* hooks works for TF => 0.12. This is due to changes, which come with release of Terraform 0.12 and `terraform-docs` not updated to support those changes.
...
* `terraform_validate_no_variables` - Validates all Terraform configuration files.
