...
4. To exclude a path from validation use the `--exclude-path` parameter. The parameter can be used multiple times.
...
5. To use a temporary Terraform data directory and not the default `.terraform` directory (in the path where the validation is performed), you can use the  `--use-temp-data-dir` parameter. `terraform_validate` will then create a temporary directory, perform a `terraform init` and finally starts the validation process. 
...
## Notes about terragrunt_validate hooks

1. To exclude a path from validation use the `--exclude-path` parameter. The parameter can be used multiple times.
