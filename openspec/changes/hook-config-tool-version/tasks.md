...
- [x] 1.11 Implement `--hook-config=--tool-version-mode=strict|prefer-local` (default `strict`), read directly by `common::resolve_tool_version` - `prefer-local` uses an already-on-`PATH` binary as-is and skips the cache/download path entirely
...
- [x] 1.13 Refactor `common::per_dir_hook` to accept the caller's resolved `tool_path` as an explicit second positional parameter (instead of hooks communicating it via an implicit global variable inherited by `common::per_dir_hook`'s backgrounded subshells), threaded through to both `per_dir_hook_unique_part` and `run_hook_on_whole_repo`; update all 13 hooks that call `common::per_dir_hook` accordingly, including the three Terraform-consuming ones, which now call `common::get_tf_binary_path` from their own `main()` instead of `common::per_dir_hook` doing it internally on every hook's behalf

## 2. Per-hook wiring — all eligible hooks in one pass

- [x] 2.1 Wire `terraform_tflint.sh`
- [x] 2.2 Extend `common::get_tf_binary_path`'s precedence chain to consult the resolver for Terraform when `--tool-version` is set (new rung: below `--tf-path`, above env vars / plain `PATH` lookup); called explicitly from `terraform_validate.sh`/`terraform_fmt.sh`/`terraform_providers_lock.sh`'s own `main()` (per task 1.13) - the only three hooks that consume its result - so other hooks' unrelated `--tool-version` settings are never misread as a Terraform pin
- [x] 2.3 Wire `terraform_docs.sh`
- [x] 2.4 Wire `terraform_trivy.sh` and `terraform_tfsec.sh`
- [x] 2.5 Wire `terrascan.sh`
- [x] 2.6 Wire `tfupdate.sh`
- [x] 2.7 Wire `terragrunt_providers_lock.sh` / other terragrunt-binary-dependent hooks
- [x] 2.8 Wire `infracost_breakdown.sh`
- [x] 2.9 Confirm `terraform_checkov.sh` deliberately does NOT call the resolver (documents the pip-distribution exclusion in code comments)
- [x] 2.10 Verify the "same hook, multiple pinned versions in one config" scenario end-to-end for at least two of the wired tools
...
- [ ] 4.2 Manual test: inside this project's Docker image, same scenario — confirm no `Dockerfile`/`.dockerignore` changes were needed (not run this session — requires a full multi-stage `docker build`, not exercised in this sandbox)
...
- [ ] 4.7 Confirm `docker build --build-arg <TOOL>_VERSION=...` still works unmodified after the `tools/install/_common.sh` conditional-sourcing change (not run this session — no root access in this sandbox to create `/.env` and simulate the build-time branch directly; the change is a pure `[[ -f /.env ]] &&` guard in front of the untouched original line, verified correct by inspection)
