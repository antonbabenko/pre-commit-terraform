...
RUN if [ "${TERRAFORM_VERSION}" = "latest" ]; then \
        TERRAFORM_VERSION="$(curl -s --insecure https://api.github.com/repos/hashicorp/terraform/releases/latest | grep tag_name | grep -o -E -m 1 "[0-9.]+")" \
    ; fi && \
    curl -L --insecure "https://releases.hashicorp.com/terraform/${TERRAFORM_VERSION}/terraform_${TERRAFORM_VERSION}_linux_amd64.zip" > terraform.zip && \
...
RUN . /.env && \
    ...
    ( \
        ...
        [ "$INFRACOST_VERSION" = "latest" ] && curl -L --insecure "$(curl -s --insecure ${INFRACOST_RELEASES}/latest | grep -o -E -m 1 "https://.+?-linux-amd64.tar.gz")" > infracost.tgz \
        || curl -L --insecure "$(curl -s --insecure ${INFRACOST_RELEASES} | grep -o -E "https://.+?v${INFRACOST_VERSION}/infracost-linux-amd64.tar.gz")" > infracost.tgz \
...
RUN . /.env && \
    ...
    ( \
        ...
        [ "$TERRAFORM_DOCS_VERSION" = "latest" ] && curl -L --insecure "$(curl -s --insecure ${TERRAFORM_DOCS_RELEASES}/latest | grep -o -E -m 1 "https://.+?-linux-amd64.tar.gz")" > terraform-docs.tgz \
        || curl -L --insecure "$(curl -s --insecure ${TERRAFORM_DOCS_RELEASES} | grep -o -E "https://.+?v${TERRAFORM_DOCS_VERSION}-linux-amd64.tar.gz")" > terraform-docs.tgz \
...
RUN . /.env \
    ...
    ( \
        ...
        [ "$TERRAGRUNT_VERSION" = "latest" ] && curl -L --insecure "$(curl -s --insecure ${TERRAGRUNT_RELEASES}/latest | grep -o -E -m 1 "https://.+?/terragrunt_linux_amd64")" > terragrunt \
        || curl -L --insecure "$(curl -s --insecure ${TERRAGRUNT_RELEASES} | grep -o -E -m 1 "https://.+?v${TERRAGRUNT_VERSION}/terragrunt_linux_amd64")" > terragrunt \
    ) && chmod +x terragrunt \
    ; fi
...
RUN . /.env && \
    ...
    ( \
        ...
        [ "$TERRASCAN_VERSION" = "latest" ] && curl -L --insecure "$(curl -s --insecure ${TERRASCAN_RELEASES}/latest | grep -o -E -m 1 "https://.+?_Linux_x86_64.tar.gz")" > terrascan.tar.gz \
        || curl -L --insecure "$(curl -s --insecure ${TERRASCAN_RELEASES} | grep -o -E "https://.+?${TERRASCAN_VERSION}_Linux_x86_64.tar.gz")" > terrascan.tar.gz \
...
RUN . /.env && \
    ...
    ( \
        ...
        [ "$TFLINT_VERSION" = "latest" ] && curl -L --insecure "$(curl -s --insecure ${TFLINT_RELEASES}/latest | grep -o -E -m 1 "https://.+?_linux_amd64.zip")" > tflint.zip \
        || curl -L --insecure "$(curl -s --insecure ${TFLINT_RELEASES} | grep -o -E "https://.+?/v${TFLINT_VERSION}/tflint_linux_amd64.zip")" > tflint.zip \
...
RUN . /.env && \
    ...
    ( \
        ...
        [ "$TFSEC_VERSION" = "latest" ] && curl -L --insecure "$(curl -s --insecure ${TFSEC_RELEASES}/latest | grep -o -E -m 1 "https://.+?/tfsec-linux-amd64")" > tfsec \
        || curl -L --insecure "$(curl -s --insecure ${TFSEC_RELEASES} | grep -o -E -m 1 "https://.+?v${TFSEC_VERSION}/tfsec-linux-amd64")" > tfsec \
    ) && chmod +x tfsec \
    ; fi
...
RUN . /.env && \
    ...
    ( \
       curl -L --insecure "https://github.com/katbyte/terrafmt/archive/refs/tags/v0.3.0.zip" > terrafmt.zip \
